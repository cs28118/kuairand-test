...
def build_proposal_prompt(config: BenchmarkConfig, goal: str) -> str:
    ...
    schema = json.dumps(experiment_spec_schema(), indent=2, sort_keys=True)
...
- Use an argv command beginning with `python` or `python3`, followed by one allowed relative .py file.
...
- Propose a small, reviewable git diff. An empty diff is allowed only when an existing experiment script is sufficient.
...
Return ONLY one JSON object conforming exactly to this ExperimentSpec JSON Schema. No Markdown, prose, code fence, or explanation:
{schema}
"""

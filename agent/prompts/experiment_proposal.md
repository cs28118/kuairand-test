...
- Propose a small, complete, reviewable unified git diff. An empty diff is allowed when an existing experiment script is sufficient; if you cannot write a complete patch, use `git_diff: ""` and run an existing script such as `experiments/run_date_dow_fm.py`. Never emit a partial or pseudo-diff.
...
JSON and patch rules (mandatory):
- Emit strict JSON: double quotes around every key and string, no comments, no trailing commas, and no duplicate keys.
- Do not put literal line breaks inside a JSON string. Encode patch line breaks as the two characters `\\n`.
- `git_diff` must be either the empty string or a complete patch beginning with `diff --git a/<path> b/<path>`, followed by valid `---`, `+++`, and hunk lines. A code snippet beginning with `+def`, `-line`, or `@@` alone is invalid.
- If you cannot produce that complete patch exactly, set `git_diff` to `""` and use an existing script. Do not invent a command path.
- result_file must be exactly `experiment_result.json`, and result_compare and next_steps must each be JSON strings.
